...
---
= Genera un piano di aggiornamento ONTAP per cluster singoli e multipli in Digital Advisor
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Prima di generare un piano di aggiornamento, dovresti:

* Assicurarsi che AutoSupport (ASUP) sia abilitato e configurato correttamente, e che siano incluse tutte le sezioni richieste. Per ulteriori informazioni, consultare link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["Guida alla risoluzione di ONTAP AutoSupport"].
* Verificare che i membri del cluster AutoSupport corrispondano alla configurazione del cluster SAP.  Per maggiori informazioni,link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["aprire un caso non tecnico"] .
...
.Singolo cluster
--
. Nella dashboard, seleziona il numero di cluster nel widget *Upgrade Advisor*.
+
...
. Seleziona un cluster per l'upgrade. La colonna *Target OS Version* mostra la versione del sistema operativo di destinazione consigliata. Puoi selezionare l'icona image:edit_icon.png["Immagine del pulsante Modifica"] per scegliere un'altra versione del sistema operativo di destinazione per un cluster.
+
...
+
** Puoi selezionare il numero di nodi nella colonna *Nodi* per vedere il riepilogo dei nodi di un cluster.
** Puoi selezionare *Risolvi problemi* dalla colonna *Azione consigliata* per risolvere i problemi di un cluster non idoneo e renderlo idoneo per un aggiornamento. + image:r_ua_resolve_issue.png["Un'immagine che mostra il tipo di problema e la relativa risoluzione."]


. Seleziona *Genera piano di aggiornamento*. Verrai reindirizzato alla pagina *Upgrade Recommendation*.
...
** Puoi selezionare l'icona image:ua_export_icon.png["pulsante export (esporta)"] per esportare il riepilogo dei rischi in un foglio Excel.


. Seleziona *Genera piano di aggiornamento* dalla pagina *Raccomandazione di aggiornamento*.
...
. Seleziona *Genera*. Verrai reindirizzato alla pagina *Report*.
. È possibile scaricare il piano di aggiornamento dalla pagina *rapporti* una volta disponibile.
+

TIP: Puoi selezionare *Visualizza report di Upgrade Advisor* per accedere alla pagina *Report*. + image:r_ua_view_reports.png["Uno screenshot che mostra la posizione in cui si trova View Upgrade Advisor per la selezione."]
...
. Seleziona i cluster da aggiornare. La colonna *Versione del sistema operativo di destinazione* visualizza la versione del sistema operativo di destinazione consigliata. Puoi selezionare l'icona image:edit_icon.png["Immagine del pulsante Modifica"] per scegliere un'altra versione del sistema operativo di destinazione per un cluster.
+
image:r_ua_select_target_OS_version.png["Un'immagine che mostra come selezionare la versione di destinazione."]

+
** Puoi selezionare il numero di nodi nella colonna *Nodi* per vedere il riepilogo dei nodi di un cluster.
** Puoi selezionare *Risolvi problemi* dalla colonna *Azione consigliata* per risolvere i problemi dei cluster non idonei e renderli idonei per un upgrade. + image:r_ua_resolve_issue.png["Un'immagine che mostra il tipo di problema e la relativa risoluzione."]


. Seleziona *Genera Upgrade Plan*.
. Fornire i dettagli nella finestra a comparsa visualizzata.
  +
